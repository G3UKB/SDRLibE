...
int conn_udp_init() {
	...
	udp_conn_td = (UDPConnThreadData *)safealloc(sizeof(UDPConnThreadData), sizeof(char), "CONN_UDP_TD_STRUCT");
...
static char* c_conn_enum_audio_outputs(cJSON *params) {
	...
	*/
	...
	cJSON_AddItemToObject(root, "outputs", outputs = cJSON_CreateArray());
		...
		items = cJSON_CreateObject();
		cJSON_AddStringToObject(items, "name", "A device");
		cJSON_AddStringToObject(items, "api", "An API");
		cJSON_AddNumberToObject(items, "index", 0);
		cJSON_AddNumberToObject(items, "direction", 0);
		cJSON_AddNumberToObject(items, "channels", 2);
		cJSON_AddItemToArray(outputs, items);
